...
function playMatchAgain() {
  ...
  while (true) {
    addVerticalSpace();
    prompt("Do you want to play another: match Yes (Y) or No (N)");
    answer = readline.question().trim().toLowerCase();
...
function trackScore (scores, player, dealer) {
  scores.playerScore += player;
  console.log(scores.playerScore);
  scores.dealerScore += dealer;
  console.log(scores.playerScore);
}
...
while (true) {
  ...
  welcomeMessage();
  const scores = {playerScore: 0, dealerScore: 0};

  while (scores.playerScore < 5 && scores.dealerScore < 5) {
    console.clear();
    ...
    while (true) {
      ...
      if (stayOrHit() === "h") {
        ...
        if (playerTotal > 21) {
          console.clear();
          bust (playerTotal, dealerTotal, scores);
          displayHand(playerHand);
          addVerticalSpace();
    ...
    prompt(`The Match Scores: Player: ${scores.playerScore}, Dealer: ${scores.dealerScore}`);
    readline.question();
  }
  if (playMatchAgain() === "n") break;
}
